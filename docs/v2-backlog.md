...
- **範圍**：
  ...
  - **go-live 前置（ops）**：真實 OA channel token、移車/釋出/取消文案定稿、per-member `line_id` 綁定流程；緊急/其他版本（B/C/D）文案；admin/staff 取消措辭。
- **依賴（卡點）**：
