...
- **Phase 5 開始（go-live readiness，規劃見 [go-live-readiness.md](go-live-readiness.md)）**：**✅ Slice 5A — LINE webhook + pending binding 擷取（2026-07-04）**（handoff §6.19：驗簽 raw-body HMAC + capture-only 零回覆 + `0018` `pending_binding` 一帳號一 active pending upsert；只建 pending 申領、**不寫 `users.line_id`**；可安全對正式 OA dry-run）。**下一刀 5B：pending 審核 → 寫 `users.line_id`（遵守 `users_line_id_key` partial unique），之後配真 OA token 即可真送達。ops 軌並行：真實 OA channel token + 文案定稿 + OA 加入率。**
