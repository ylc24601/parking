...
1. 先給測試會友綁 `line_id`：手機加測試 OA → 傳 `綁定 <code>`（`binding:issue` 發碼）→ `binding:approve --apply`。
2. 手機開 LIFF URL（`https://liff.line.me/<LIFF_ID>`）→ 應直接進入本週狀態卡（不再看到登入畫面）。
3. 驗證項目：
   - [ ] 已綁定會友 → 狀態卡正確（週日期/狀態/車牌/期限，台北時間）
   - [ ] 未綁定 LINE 帳號 → `not_bound` 畫面（不建 session、不自動綁）
   - [ ] 關閉 LIFF 重開 → session 仍在（cookie 30 天）
   - [ ] 登出 → 回 gate；重進 → LIFF 自動重驗後恢復
   - [ ] server log 無 ID token / userId 洩漏
4. 完成後把結果記回 handoff §6.23（比照 5B pilot 紀錄格式）。
