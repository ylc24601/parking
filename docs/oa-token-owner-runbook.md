...
- [ ] **真的發一則通知，但走系統既有的單次測試安全閥，不要對真會友亂送**：系統預設 `LINE_SEND_ENABLED=false`（送出鎖，就算 transport 設定為正式送出，這道鎖沒開一律不送）。正確流程是開發者把它**暫時翻成 `true`**、只對**一位知情且同意的內部人員自己的帳號**送一則測試通知、確認手機真的收到，**再翻回 `false`**。這樣即使測試沒做好，最多打擾到一個知情的自己人，不會誤發給會友。
...
- [ ] 確認新 token 生效後，回到 LINE Developers Console **明確執行撤銷（revoke）舊 token**——LINE Developers Console 的發行（issue）跟撤銷（revoke）是**兩個分開的動作**，**不要假設「發一組新的」會自動讓舊的失效**，兩者必須都做。
- [ ] **撤銷後要驗證舊 token 真的失效**：拿舊 token 對 LINE API 打一次任意需要驗證的請求（例如查 profile 或發訊息 API），預期應該被拒絕。沒驗證過的撤銷，等於沒撤銷。
- [ ] 注意：光是把系統設定（`NOTIFICATION_TRANSPORT`）從測試模式切到正式模式，**不會讓舊 token 失效**——舊 token 若沒手動撤銷並驗證，仍然是有效的、可被濫用的金鑰。
