...
>
> **Wave 2C-1（#19）起帳號分兩級**：**系統管理員（superadmin）** 可用全部後台；**幹事（clerk）** 不能開「帳號管理／通知系統狀態／稽核記錄」三頁，其餘日常營運照舊。

---

## 建立帳號

**日常請走後台**：Wave 2C-2 起 `/admin/accounts` 有「新增管理者」——填帳號、顯示名稱、選角色（**預設幹事**），建立後畫面顯示一次性密碼（只顯示這一次）。建立**系統管理員**時會跳出列出完整權限的高權限警告，要再確認一次。此路徑**會寫稽核**（`admin_account.create`）。
...
| **變更角色** | 系統管理員 ⇄ 幹事互換（**改為系統管理員時跳高權限警告**）+ **變更後該帳號所有裝置登出**（新角色下次登入即生效）。同角色再送是無效果的 no-op，不寫紀錄 |

- **只有系統管理員能用這一頁**：幹事開啟會看到「權限不足」，直接打 API 也會被擋（403）。擋的地方三層——**側欄不顯示（只是 UX）＋頁面/API server-side 檢查＋DB RPC 在交易內自己重讀角色再擋一次**（角色從不由呼叫端聲稱）。**五個寫入操作（停用／重設密碼／撤銷 session／變更角色／新增帳號）全部走 audited RPC**；2C-2 起「撤銷 session」也補上了 RPC 與稽核（不再是先前的直接 DELETE）。
- **自己的帳號沒有這些按鈕**：任何人都不能對自己執行。self-target 由 RPC 判定並**寫一列 `denied` 稽核**（不論從 UI 或直接打 API，結果一致）。要結束自己的 session 用「登出」。
- **系統永遠保留至少一位啟用中的系統管理員**。這是**結構性**保證：能執行這些操作的人自己必須是「啟用中的系統管理員」，而且不能對自己下手 ⇒ 停用或降級他人後他本人一定還在。兩位系統管理員同時互相停用/降級時，advisory lock 會把呼叫排成先後，後到的那位發現自己已被停用而被拒（`acting_admin_disabled`），系統仍留一位。
