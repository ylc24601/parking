...
- **只有系統管理員能用這一頁**：幹事開啟會看到「權限不足」，直接打 API 也會被擋（403）。目前（2C-1）擋的地方是**頁面與 API 的 server-side 檢查**；其中「停用／重啟」與「重設密碼」另外還有 DB RPC 在交易內自己重讀角色再擋一次（角色從不由呼叫端聲稱）。
  - **「撤銷所有 session」目前只有 API 與 service 兩層**——它還是直接刪 `admin_sessions` 的 repository 操作，沒有 RPC、也**不寫稽核記錄**。這是已知缺口（要補得新開一支 RPC），不是漏寫。
  - **側欄還不會依角色隱藏**（Wave 2C-2 才做）。在那之前幹事看得到那三個項目，點進去會看到「權限不足」。
- **自己的帳號沒有這些按鈕**：任何人都不能停用/重設/撤銷自己。停用與重設密碼是 route／service／RPC 三層擋；撤銷 session 是 route／service 兩層。要結束自己的 session 用「登出」。
