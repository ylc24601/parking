...
5. **Smoke the staged deployment at its own URL**, before it serves anyone. It already has
   ...
   - **Open it in a browser, signed in to the Vercel account.** **Under this project's current
     Deployment Protection setting**, the autogenerated domains of a staged deployment sit
     behind Vercel Authentication and 302 to `vercel.com/sso-api`, so `curl` and other
     non-browser tools get the redirect rather than the app. *Measured 2026-07-27:* all three
     staged domains (project alias, `*-git-main-*` branch alias, unique `*-<hash>-*`
     deployment URL) redirected to SSO; only the production domain answered 200.
   - **This is a project setting, not an inherent property of staged deployments.** Protection
     scope is configured per project and Vercel has changed its defaults over time, so treat
     the line above as a dated measurement, not a platform guarantee — **re-check it rather
     than assume it**. That protection is *desirable*: a staged build carries Production env —
     real Supabase credentials — and must not be publicly reachable before promotion. **A
     staged domain answering 200 to an unauthenticated request is a finding to act on, not a
     convenience.**
