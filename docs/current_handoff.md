...
- **Phase 8 後續 slices**：**② 會友管理＋發碼 UI（本刀 §6.29 完成）** → ③ 資格審查＋admin 帳號管理（停用/改密/`deleteAdminSessionsByAdminId` 全裝置撤銷）④ CSV 匯入上傳（`memberImportService` 抽 buffer 變體）⑤ 營運可視化 ⑥ **PII retention job（必收項**，決行 90 天清 `claimed_phone`/`claimed_name`/`submitted_code`，見 binding-ops.md**）** ⑦ 牧養處理＋Staff PIN 管理 UI。
