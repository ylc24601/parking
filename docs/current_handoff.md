...
> 最後更新：2026-07-30 ｜ **Phase 9 已收官；production demo walkthrough 完成並清回 baseline；正式教會會友名冊已於 2026-07-28 匯入 prod（57 位 ＝ CSV 54 ＋ 手動自建 3）**。Post-Phase-9 功能軌（[feature-triage.md](feature-triage.md)）：Wave -1/0/0.1/1 ✅、**Wave 2A #15 稽核全完成（2A-1／2A-2／2A-3 retention）、Wave 2B-1 #14A ✅、Wave 2B-2a＋2B-2b #10 ✅ ⇒ 容量／P2 資格不需 SQL／CSV，稽核有邊界可清理**、**Wave 2C #19 admin 角色分級 ✅（2C-1／2C-2）**、**Wave 3 ✅（3a #8＋#9／3b #17／3c #18／3d #5B-a）**。**「交付前必修」與「強烈建議交付前」兩節皆已清空 ⇒ 開發面無交付阻擋**（剩交付後 ops，走查照 **[go-live-checklist.md](go-live-checklist.md)**＝單一權威清單）；驗證見 §8。**Wave 3 3e #32 概覽補申請狀況＋優先·一般拆解 ✅（PR #59）**。**active 剩片**（2026-07-30 更新，權威為 [feature-triage.md](feature-triage.md) 的 Feature inventory）：**#33a 眷屬年齡／學齡前**（`Ready`／`Pre-delivery`，交付前唯一剩項）、**#35 本週申請清單**（`Ready`／`Pre-pilot`）、**#34 的 34-0／34-0b-A**（`Ready`／`Pre-pilot`）；**#14B 申請開放 override** 仍 `Blocked`（規則未定、待產品決策，非技術阻擋）。 ｜ 範圍：Phase 0–2 全部、Phase 3（Staff 現場頁 + v2 全切片）、Phase 4（notification dispatcher A–F）、Phase 5/5B（LINE webhook + binding 擷取/審核/CLI）、Phase 6（會友資料匯入）、Phase 7（會員 LIFF：登入/綁定/申請/取消/遞補確認/正在路上）、Phase 8（Admin UI Slice 1–8）、Phase 9（production deploy + prod demo-complete 收官）全數完成——詳見 §6.13–§6.36；post-Phase-9 各刀見 §6.37–§6.48。
>
> **現況：開發面無交付阻擋、prod 已站起並跑完完整 demo（見 §6.36）、staged deployment 部署制度已上線並實跑驗收（見 §6.48／§6.48.1）。剩交付後 ops（非開發軌）**：**只剩 [go-live-checklist.md](go-live-checklist.md) §1.7 pilot 分批放行**（§0 三個負責人已於 2026-07-29 指派，具名記錄不入本公開 repo；⚠️ **三角色目前由同一人兼任、無備援 on-call，pilot 擴大前需補一位**）。**資料備份已完成、非待辦**（決策 2026-07-18 先不升 Pro、走 Free ＋自管加密備份；**2026-07-20 即已 arm，每晚實跑、還原演練通過**；2026-07-29 以 `gh variable list`／`gh run list` 實查確認——go-live §1.1 一度仍列六項「待教會填」為 stale，已更正）。**§1.3 匯入真會友 CSV 已於 2026-07-28 完成**（57 位；殘留名冊面待辦＝4 位 `reviewRequired` 待設效期、1 位缺手機未匯入、其餘跳過者於宣導綁定時人工處理——見 go-live §1.3 與 [feature-triage.md](feature-triage.md) #34 專節）。**已完成的 ops**：staged deployment（§1.0）、教會正式 OA/channel token（§1.2／§6.38）、通知文案 sign-off（§1.4／§6.39）、12 個 cron 含 audit purge（§1.5／§6.40）、開啟真實送出 `NOTIFICATION_TRANSPORT=line`（§1.6／§6.41）。**交付日照 [go-live-checklist.md](go-live-checklist.md)**（單一權威走查清單，整合 runbook §8/§13＋go-live-readiness §1/§5）。非阻擋功能 backlog（#26 通知 LIFF deep-link、2B-2c、#5B-b／#5B-c、a11y）見 [feature-triage.md](feature-triage.md) 與 [pre-delivery-polish-backlog.md](pre-delivery-polish-backlog.md)；**`server-only` 邊界已於 Tier 0-1（PR #54）完成，見 §6.49；會友資料維護（單筆新增／改姓名手機／車輛停用恢復＋匯入身分守衛）已於 Tier 0-2（migration `0038`）完成，見 §6.50——這一刀帶 migration，**A⚠️ almost**（LIFF 審核理由改名，舊 app 在 migration→Promote 之間會 500，資料無風險）／B❌，prod 必須先 `db push` 再 promote**。
...
**UI 視覺走查（本機 headless Chrome ＋ puppeteer-core 驅動，PR #46 第一輪審查後補跑，親眼看過每一個 client 狀態）**——全部渲染正確：帳號管理頁（sidebar 11 項、新增管理者表單、角色欄 badge、self-row 無操作鈕只顯示「目前登入」）✅；**選系統管理員 → 紅色高權限警告**列出實際權限（「可管理其他管理者帳號、查看稽核記錄及操作營運維護功能」）✅；建立幹事 → 確認框 → **一次性密碼區塊**（密碼碼框＋複製鈕＋修正後文案「…若密碼遺失，需請系統管理員重設」，**無「自行更換」**）✅，且**新帳號即時出現在下方清單**（權威 DTO state 同步）＋帶「改為系統管理員」等列內操作 ✅；**重複 username → 錯誤橫幅＋「重新載入帳號清單」按鈕** ✅；手機寬度（390px）表單縱向堆疊、導覽橫向捲動、表格於 overflow 容器內捲動 ✅；`/admin/audit` 實際渲染「（當時身分：系統管理員）」「已刪除管理員（…）（當時身分：幹事）」、系統列**無**角色後綴、「新增管理員帳號」中文 label ✅。（截圖存 session scratchpad；puppeteer 裝在 scratchpad 專案、未動 repo。）
...
**#19（Wave 2C）完成**：程式、自動測試、server 端 runtime 走查、以及 UI 視覺走查皆通過。剩雲端 smoke 屬**交付當天**的 DB-first 部署驗證（runbook §1.5 第 4 步），非本刀範圍。**prod 目前仍沒有任何 clerk 帳號**，交付時由第一位系統管理員在 `/admin/accounts` 建立幹事。
...
## 6.45 Wave 3 slice 3b — 通知系統狀態 B＋C（#17，2026-07-25）

Wave 3 第二刀。`/admin/ops`「營運狀態」→**改名「通知系統狀態」**並重構呈現；概覽讓幹事看到白話通知健康。**無 migration**（純呈現＋DTO/service 層），db:verify 仍 48。路由不變（只改顯示名），未動 page config／route/layout 契約。
...
**讀取＝keyset（非 offset）**：`repo.listMembersForExportPage` 以 immutable `(created_at, id)` 游標＋`created_at < createdBefore` cutoff。**契約精確**：keyset 保證不因 offset 位移造成既有資料 dup/skip；DB cutoff 固定一個穩定上界——**非嚴格 txn snapshot**（各頁是分開的 statement）。**cutoff 來自 DB clock**（`0037 db_now`／`repo.getDbNow`，與 `users.created_at` 同一時鐘；不用 app/Vercel clock 以免 skew 誤判邊界；審查必修 2）。**游標傳 RAW created_at 字串、不經 Date**（會截 microsecond、破 tied-ts）。PostgREST `.or()` 內時間戳**雙引號**才不把 `:`/`+` 當 filter 語法（已實測）。**active-plate 附掛共用 helper 依 `PLATE_LOOKUP_CHUNK=100` 分 chunk**（`.in()` 幾百 UUID 會 414；審查必修 1）＋與 `listMembers` 共用（`is_active` 不 drift）。

**服務順序（audit 邊界）**：`requireAdminActor(actor)`（不信任 plain AuditActor、驗 actor type＋narrow ids；審查必修 3）→ keyset 讀到底 → 記憶體 `(display_name,id)` 排序 → **先完整 serialize CSV** → 才 DB reauth+audit（0037）→ 才回。CSV formula guard 含全形 `＝＋－＠`（CJK 資料、OWASP）。故 serialize 失敗不留 false-success audit；reauth 拒絕（中途降權）回 typed `forbidden`→route 403、不回 CSV。`requestId` 由 route 產生下傳。client：**先檢 `res.ok`**（403 JSON 不當 csv 下載）＋blob download 自設 `a.download`（blob: URL 不繼承 Content-Disposition，filename parser 只收 `members-\d{8}\.csv`）。
