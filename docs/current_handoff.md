...
- **Phase 8 後續 slices**：**② 會友管理＋發碼（§6.29）③ admin 帳號管理（§6.30）④ P2 資格審查唯讀檢視（§6.31）⑤ CSV 匯入上傳（§6.32）⑥ 營運狀態（§6.33）⑦ PII retention job（必收項，§6.34）⑧ 牧養處理＋現場 PIN 管理 UI（§6.35 完成）**——Phase 8 slice map 全數完成；後續 follow-up：資格審查**寫入型覆核**（需 `reviewed_by_admin_id` migration，§6.31 follow-up）；匯入稽核欄（誰匯入，§6.32 follow-up）。
...
**Slice 4 走查（本回合實跑，2026-07-15，runbook §12.1–12.3 逐步引導）**：6 位合成 P2 cohort（`DEMO` marker、保留 09 號段、`DEMO0x` 車牌，過正式 validator）＋開發者自己的 LINE 綁定身分為 live member，在**遠期專用 demo event**（`2026-12-27`，`blocked_spaces=21`→effective capacity 2）跑完整業務鏈。逐項 **PASS**：
...
- **PII leak check（兩層）PASS**：Layer A `notification_outbox.last_error` 僅 `no_line_id`（零 PII）；Vercel Hobby 無可搜尋 runtime log→採 fallback（log-payload sanitization 由 test-suite leak-scan 斷言佐證）；Layer B role-boundary 各 surface（Member 自己卡／Staff 點名表／Admin）觀察無禁見欄位外洩。
