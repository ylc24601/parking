...
> 最後更新：2026-07-29 ｜ **Phase 9 已收官；production demo walkthrough 完成並清回 baseline；正式教會會友名冊已於 2026-07-28 匯入 prod（57 位 ＝ CSV 54 ＋ 手動自建 3）**。Post-Phase-9 功能軌（[feature-triage.md](feature-triage.md)）：Wave -1/0/0.1/1 ✅、**Wave 2A #15 稽核全完成（2A-1／2A-2／2A-3 retention）、Wave 2B-1 #14A ✅、Wave 2B-2a＋2B-2b #10 ✅ ⇒ 容量／P2 資格不需 SQL／CSV，稽核有邊界可清理**、**Wave 2C #19 admin 角色分級 ✅（2C-1／2C-2）**、**Wave 3 ✅（3a #8＋#9／3b #17／3c #18／3d #5B-a）**。**「交付前必修」與「強烈建議交付前」兩節皆已清空 ⇒ 開發面無交付阻擋**（剩交付後 ops，走查照 **[go-live-checklist.md](go-live-checklist.md)**＝單一權威清單）；驗證見 §8。**active 剩片**（2026-07-30 更新，權威為 [feature-triage.md](feature-triage.md) 的 Feature inventory）：**#32 概覽補申請狀況＋P2·P3 拆解**與 **#33a 眷屬年齡／學齡前**（皆 `Ready`／`Pre-delivery`）、**#35 本週申請清單**（`Ready`／`Pre-pilot`）、**#34 的 34-0／34-0b-A**（`Ready`／`Pre-pilot`）；**#14B 申請開放 override** 仍 `Blocked`（規則未定、待產品決策，非技術阻擋）。 ｜ 範圍：Phase 0–2 全部、Phase 3（Staff 現場頁 + v2 全切片）、Phase 4（notification dispatcher A–F）、Phase 5/5B（LINE webhook + binding 擷取/審核/CLI）、Phase 6（會友資料匯入）、Phase 7（會員 LIFF：登入/綁定/申請/取消/遞補確認/正在路上）、Phase 8（Admin UI Slice 1–8）、Phase 9（production deploy + prod demo-complete 收官）全數完成——詳見 §6.13–§6.36；post-Phase-9 各刀見 §6.37–§6.48。
>
