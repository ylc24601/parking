...
- **Phase 8 後續 slices**：**② 會友管理＋發碼 UI（§6.29 完成）③ admin 帳號管理（本刀 §6.30 完成）** → ④ 資格審查（P2 到期檢視/覆核，UI 唯讀先行）⑤ CSV 匯入上傳（`memberImportService` 抽 buffer 變體）⑥ 營運可視化 ⑦ **PII retention job（必收項**，決行 90 天清 `claimed_phone`/`claimed_name`/`submitted_code`，見 binding-ops.md**）** ⑧ 牧養處理＋Staff PIN 管理 UI。
