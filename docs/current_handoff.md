...
- **Phase 8 後續 slices**：**② 會友管理＋發碼（§6.29）③ admin 帳號管理（§6.30）④ P2 資格審查唯讀檢視（§6.31）⑤ CSV 匯入上傳（§6.32）⑥ 營運狀態（§6.33）⑦ PII retention job（必收項，§6.34 完成）** → ⑧ 牧養處理＋Staff PIN 管理 UI；資格審查**寫入型覆核**（需 `reviewed_by_admin_id` migration，§6.31 follow-up）；匯入稽核欄（誰匯入，§6.32 follow-up）。
