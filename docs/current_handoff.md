...
- **Phase 8 後續 slices**：**② 會友管理＋發碼（§6.29）③ admin 帳號管理（§6.30）④ P2 資格審查唯讀檢視（本刀 §6.31 完成）** → ⑤ CSV 匯入上傳（`memberImportService` 抽 buffer 變體）⑥ 營運可視化 ⑦ **PII retention job（必收項**，決行 90 天清 `claimed_phone`/`claimed_name`/`submitted_code`，見 binding-ops.md**）** ⑧ 牧養處理＋Staff PIN 管理 UI；資格審查**寫入型覆核**（需 `reviewed_by_admin_id` migration，§6.31 follow-up）。
