...
describe.skipIf(!RUN)('binding CLI service (5B-2) — local DB integration', () => {
  ...
  it('issue → preview (masked, no write) → apply (writes line_id) → idempotent re-apply', async () => {
    ...
    // Apply: writes line_id, consumes code, marks approved.
    expect(await svc.applyApproveBinding({ pendingId: pid, now: NOW }, repo)).toEqual({ approved: 1, reason: 'approved' })
    ...
    expect(await svc.applyApproveBinding({ pendingId: pid, now: NOW }, repo)).toMatchObject({ approved: 0, reason: 'pending_not_pending' })
  })
